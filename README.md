...
### Основная версия (оптимизированная) - `cmd/main.go`
...
### Простая версия - `cmd/simple/main.go`
...
## 🚀 Быстрый старт с PostgreSQL (пошагово для новичков)
...
### 2. Создай пользователя и базу данных

```bash
sudo -u postgres psql
```
Внутри psql (появится приглашение postgres=#):
```sql
CREATE USER messenger WITH PASSWORD 'securepassword';
CREATE DATABASE messenger_db OWNER messenger;
\q
```

### 3. Проверь владельца схемы public (должен быть messenger)

```bash
sudo -u postgres psql -d messenger_db -c "\dn+"
```
В колонке Owner для public должно быть `messenger`.

### 4. Примени миграции (создай таблицы)

```bash
psql -U messenger -d messenger_db -h 127.0.0.1 -W -f migrations/001_init_postgres.sql
```

### 5. Настрой файл .env для PostgreSQL
...
- Проверь, что все команды выполнялись без ошибок.
- Проверь, что в .env нет опечаток.
- Если что-то не работает — смотри сообщения в терминале, копируй ошибку и ищи решение или спрашивай у сообщества.

---

**Теперь твой мессенджер работает на PostgreSQL и готов к высокой нагрузке!**
